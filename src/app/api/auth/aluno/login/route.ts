import { NextRequest, NextResponse } from 'next/server'
import { loginSchema } from '@/lib/validations/auth.schema'
import {
  comparePassword,
  signAccessToken,
  signRefreshToken,
  COOKIE_ACCESS,
  COOKIE_REFRESH,
  COOKIE_ACCESS_OPTIONS,
  COOKIE_REFRESH_OPTIONS,
} from '@/lib/auth'
import { prisma } from '@/lib/prisma'

export async function POST(request: NextRequest) {
  try {
    const body = await request.json()
    const parsed = loginSchema.safeParse(body)

    if (!parsed.success) {
      return NextResponse.json({ error: 'Dados inválidos' }, { status: 400 })
    }

    const { email, password } = parsed.data

    const student = await prisma.student.findFirst({
      where: { email },
      select: { id: true, name: true, email: true, passwordHash: true, status: true },
    })

    if (!student || !student.passwordHash) {
      return NextResponse.json({ error: 'Credenciais inválidas' }, { status: 401 })
    }

    if (student.status === 'INACTIVE') {
      return NextResponse.json({ error: 'Conta inativa. Fale com seu personal.' }, { status: 403 })
    }

    const valid = await comparePassword(password, student.passwordHash)
    if (!valid) {
      return NextResponse.json({ error: 'Credenciais inválidas' }, { status: 401 })
    }

    const tokenPayload = { sub: student.id, email: student.email!, role: 'student' as const }
    const [accessToken, refreshToken] = await Promise.all([
      signAccessToken(tokenPayload),
      signRefreshToken(tokenPayload),
    ])

    const response = NextResponse.json(
      { data: { student: { id: student.id, name: student.name, email: student.email } } },
      { status: 200 }
    )

    response.cookies.set(COOKIE_ACCESS, accessToken, COOKIE_ACCESS_OPTIONS)
    response.cookies.set(COOKIE_REFRESH, refreshToken, COOKIE_REFRESH_OPTIONS)

    return response
  } catch {
    return NextResponse.json({ error: 'Erro interno do servidor' }, { status: 500 })
  }
}
